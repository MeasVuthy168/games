...
async function renderList() {
  ...
  listView.innerHTML = `
    <div class="chat-list" id="chatList"><div class="empty-note">Loading…</div></div>
    <div class="msg-menu-overlay" id="convMenuOverlay" hidden>
      <div class="msg-menu" id="convMenu"></div>
    </div>
  `;
  // listView IS the page-transition-root here (styles.css). The shell
  // above (including its "Loading…" placeholder) is real, stable content
  // the instant it's set -- the slide no longer waits on loadConversations()
  // below, which only replaces #chatList's own contents once it resolves.
